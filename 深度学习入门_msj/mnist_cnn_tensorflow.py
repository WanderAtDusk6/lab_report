...
def conv2d(x, W):          # 卷积神经网络层  x 输入的值， W 即Weight
    ...
    return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME')    # 二维的   strides(步长),前和后必为1，然后中间的是x,y方向，这边定的跨度是1
...
with tf.name_scope('layer_func2'):
    with tf.name_scope('W_func2'):
        W_fc2 = weight_variable([1024, 10])   # 输出10类
...
with tf.name_scope('train'):
    train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)     # Adam算法

sess = tf.Session()
writer = tf.summary.FileWriter("./", sess.graph)              # 可视化呀可视化
# important step
...
for i in range(1000):
    ...
    if i % 50 == 0:
        print("acc(%d/1000r):"%i, end=' ')
        print(compute_accuracy(
                        mnist.test.images[:1000], mnist.test.labels[:1000]))

# 关于使用tensorboard查看可视化程序图
# 打开anaconda，命令行输入
# tensorboard --logdir="F:/ML2/ " (引号内为文件地址，建议换成简单英文目录，这是我的)
# 浏览器打开连接 127.0.1.1:6006
